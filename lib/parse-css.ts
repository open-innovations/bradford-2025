import { parse, walk, render } from "jsr:@tbela99/css-parser@0.7.2";

export async function getColourVariables(cssFile, transformName=(x) => x.replace(/^--/, '')) {
    const cssText = await Deno.readTextFile(cssFile);
    const rawCss = await parse(cssText);
    const parsedCss = await parse(render(rawCss.ast, { convertColor: true }).code)
